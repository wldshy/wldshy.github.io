...
```
在公式 \eqref{eq:demo} 中，我们看到了这个被自动编号的公式。
$${E=mc^2 \text{，自动编号公式示例} label{eq:sample} }$$
```
显示：

$$在公式 \eqref{eq:demo} 中，我们看到了这个被自动编号的公式。$$

$${E=mc^2 \text{，自动编号公式示例} label{eq:demo} }$$
...
有时候要用 `\left.` 或 `\right.` 进行對其对齐而不显示本身。
...
> 若找不到需要的符号，使用 [$\rm{Detexify^2}$](https://detexify.kirelabs.org/classify.html) 来画出想要的符号。
...
对于较新的浏览器（HTML5与CSS3），额外的124种颜色将被支持。也可输入 `\color {#rgb} {text}` 来自定义更多的颜色，其中 `#rgb` 的 `r` `g` `b` 可输入 `0-9` 和 `a-f` 来表示红色、绿色和蓝色的纯度（饱和度）。
...
注意：使用方程式序列无需为每一行单独声明公式符号 `$` 或 `$$` ；因为 `{align}` 语句是 **自动编号** 的。
